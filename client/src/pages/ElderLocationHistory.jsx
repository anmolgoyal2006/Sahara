...
export default function ElderLocationHistory() {
    ...
    supabase.auth.getSession().then(async ({ data: { session } }) => {
      ...
      try {
        const [zoneRes, eventsRes] = await Promise.all([
          ...
          fetch(`${API_URL}/api/geofence/events/${uid}?type=alerts&limit=50`),
        ])
      ...
      <div style={{ maxWidth: 680, margin: '0 auto' }}>
        ...
        <div style={{ marginBottom: 20 }}>
          ...
          <p style={{ fontSize: 14, color: '#A0B8D0' }}>
            Events recorded by your Safety Zone
          </p>
        </div>
        ...
        {events.length === 0 ? (
          <div style={{
            ...
            <p style={{ fontSize: 14, color: '#A0B8D0' }}>
              Your family has not received any geofence notifications yet
            </p>
          </div>
        ) : (
        ...
        <div style={{
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 8 }}>
            ...
            <span style={{ fontSize: 13, fontWeight: 700, color: '#5A7A9A' }}>Your Privacy</span>
          </div>
          <p style={{ fontSize: 12, color: '#A0B8D0', lineHeight: 1.6, margin: '0 0 12px' }}>
            Your location is only shared with family members linked to your account.
            You can turn off the safety zone at any time from the Settings page.
          </p>
          <button
            ...
            style={{
              ...
              fontSize: 12, fontWeight: 700, color: (!zone || disabling) ? '#A0B8D0' : '#E24B4A',
